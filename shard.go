...
func ReadShardFunc(nds datastore.DataStorage, dir string) (*ShardIdV1, error) {
	buf, err := nds.Get(context.Background(), ds.NewKey(filepath.Join(dir, ShardingFn)))
	if datastore.ErrNotFound(err) {
...
func WriteShardFunc(nds datastore.DataStorage, dir string, id *ShardIdV1) error {
	err := nds.Put(context.Background(), ds.NewKey(filepath.Join(dir, ShardingFn)), []byte(id.String()))
	if err != nil {
		return err
	}

	err = nds.Put(context.Background(), ds.NewKey(filepath.Join(dir, ShardingFn)), []byte("\n"))
	return err
}
